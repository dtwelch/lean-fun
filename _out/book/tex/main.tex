...
\par
\par\noindent To prove associativity, we take \LeanVerb|P m| to be the property:\LeanVerb|(m + n) + p \PLFAEquiv{} m + (n + p)|Here \LeanVerb|n| and \LeanVerb|p| are arbitrary natural numbers, so if we can show the equation
...
The proof uses lean4's induction tactic on \LeanVerb|m|.For the base case, we must show:
