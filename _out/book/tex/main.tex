...
\par\noindent Now that we've defined the naturals and operations on them, our next step is to
...
\par\noindent Operations pop up all the time, and mathematicians have agreed on names for some
...
Here \LeanVerb|m|, \LeanVerb|n|, and \LeanVerb|p| are variables that range over all natural numbers.
...
tells us that since \LeanVerb|P zero| held yesterday, then \LeanVerb|P (suc zero)| holds today:
...
To prove associativity, we take \LeanVerb|P m| to be the property:

\LeanVerb|(m + n) + p ≡ m + (n + p)|
